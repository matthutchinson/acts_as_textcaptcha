  ...
  module Acts #:nodoc:
    module Textcaptcha #:nodoc:
      ...
      def acts_as_textcaptcha(options = nil)
        cattr_accessor :config, :textcaptcha_salt
        attr_accessor  :spam_answer, :spam_question, :possible_answers
                                
        self.textcaptcha_salt = BCrypt::Engine.generate_salt
      ...
      module InstanceMethods
        ...
        def encrypt_answer(answer)
          BCrypt::Engine.hash_secret(answer, textcaptcha_salt)
        end
        ...
        def generate_spam_question(use_textcaptcha = true)
          if use_textcaptcha && config && config['api_key']
            begin
              ...
              return possible_answers.map {|ans| encrypt_answer(ans) } if spam_question && !possible_answers.empty?
          ...
          possible_answers.map {|ans| encrypt_answer(ans) }
        end
